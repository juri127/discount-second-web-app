...
function calculateDiscounts() {
    ...
    // 入力データの取得
    const existingDiscountValue = document.getElementById('existing-discount').value;
    const existingDiscount = existingDiscountValue ? Number(existingDiscountValue) : 0;
...
function suggestDiscounts(inputData, productData) {
    ...
    console.log('suggestDiscounts内のexistingDiscount:', existingDiscount, typeof existingDiscount);
    ...
    // 合計値下げ額（既存 + 提案）
    const totalAllDiscounts = Number(existingDiscount) + Number(totalDiscountAmount);
    
    console.log('計算詳細:', {
        existingDiscount: existingDiscount,
        totalDiscountAmount: totalDiscountAmount,
        totalAllDiscounts: totalAllDiscounts,
        計算式: `${existingDiscount} + ${totalDiscountAmount} = ${totalAllDiscounts}`
    });
